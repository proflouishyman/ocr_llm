...
def generate_text_with_logging(model, dataloader, device, tokenizer, df, output_path, save_interval=10):
    model.eval()
    corrected_texts = {}
    ...
    for batch_num, batch in enumerate(tqdm(dataloader, desc="Processing batches")):
        ...
        with torch.no_grad():
            ...
            # Debugging: Print information about processed parts
            print(f"Processed batch {batch_num}:")
            for idx in original_indices:
                print(f"Index {idx}: Parts {list(corrected_texts[idx].keys())}")
...
def print_setup_details(rank, world_size):k
    if rank == 0:
...
def main():
    ...
    parser.add_argument('--output_path', type=str, default='/data/lhyman6/OCR/1919_debug4.csv', help='Path to the output')
