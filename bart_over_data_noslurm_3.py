...
def main_worker(rank, world_size, args, model_path, data_path, output_path, master_port):
    ...
    dataloader = DataLoader(dataset, batch_size=32, shuffle=False, sampler=sampler, pin_memory=True, num_workers=4)
...
def main():
    ...
    data_path = '/data/lhyman6/OCR/1919_ocr_loaded.csv'
    output_path = '/data/lhyman6/OCR/1919_15500_data_ocr_loaded_3.csv'

    world_size = torch.cuda.device_count()
    master_port = find_free_port()

    mp.spawn(main_worker, args=(world_size, args, model_path, data_path, output_path, master_port), nprocs=world_size, join=True)
